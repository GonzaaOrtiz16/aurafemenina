...
function BenefitsCarousel({ whatsapp }: { whatsapp: string }) {
  const benefits = [
    { icon: Truck, title: "ENVÍOS", subtitle: "¡Hacemos envíos a todo el país!" },
    { icon: MessageCircle, title: "ESCRIBINOS!", subtitle: `WhatsApp: ${whatsapp.slice(-10)}` },
    { icon: Tag, title: "COMPRA MÍNIMA", subtitle: "$100.000 para GBA e Interior. Sin mínimo en CABA y Zona Sur." },
  ];

  const [current, setCurrent] = useState(0);
  const next = useCallback(() => setCurrent((prev) => (prev + 1) % benefits.length), [benefits.length]);
  ...
  return (
    <div className="flex flex-col items-center text-center gap-3 min-h-[140px] justify-center">
      <div key={current} className="flex flex-col items-center text-center gap-3 animate-fade-in">
      ...
      <div className="flex gap-2 mt-2">
        {benefits.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)}
            className={`w-2 h-2 rounded-full transition-all duration-300 ${i === current ? "bg-foreground scale-110" : "bg-border"}`} />
        ))}
      </div>
    </div>
  );
...
export default function StoreInfo() {
  ...
  const c = contact || { whatsapp: "5491134944228", instagram: "@aurafemenina.oficial", instagram_url: "https://instagram.com/aurafemenina.oficial" };
      ...
      <div className="py-12 md:py-20 border-b border-border">
        <div className="container mx-auto px-6">
          <div className="hidden md:grid md:grid-cols-3 gap-16">
            ...
            <AnimatedCard icon={MessageCircle} title="ESCRIBINOS!" subtitle={`WhatsApp: ${c.whatsapp.slice(-10)}`} delay={150} />
          ...
          <div className="md:hidden">
            <BenefitsCarousel whatsapp={c.whatsapp} />
          </div>
        </div>
      </div>
        ...
        <div ref={igRef} className={`container mx-auto px-6 flex justify-center transition-all duration-700 ${igVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <a href={c.instagram_url} target="_blank"
            ...
            <div className="flex flex-col items-start">
              <p className="text-[10px] md:text-xs tracking-[0.2em] text-muted-foreground font-bold uppercase">SEGUINOS EN INSTAGRAM</p>
              <p className="text-xl md:text-4xl font-black text-foreground tracking-tighter">{c.instagram}</p>
            </div>
          </a>
        </div>
      </div>
